...
Описание всех конструкций языка Inform 6, с дополнениями, касающимися русской версии.
...
* имеет имя, по которому её нужно вызывать, `процедура()`. Также можно вызвать её не напрямую через `indirect(процедура, арг1, арг2, ... арг7)`;
* может принимать аргументы, `процедура(арг1, арг2, ... арг7)`, которые инициализируют локальные переменные;
...
* `(char) значение` — вывести символ с кодом числа.
