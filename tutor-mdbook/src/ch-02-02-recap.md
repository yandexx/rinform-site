...
У объекта может быть только один «родитель» (или не быть родительских объектов вообще), но может быть *сколько угодно* «детей» (в том числе их может и не быть).
...
> У комнат *не бывает* родительских объектов, а также одним из их дочерних объектов иногда становится игрок.
...
| 7. Игрок вводит ИДТИ ВВЕРХ               | ![](img/02-07.png) |
...
> Заметьте, что мы описали функцию ``Initialise``, но в игре мы её не вызывали. На самом деле, функция вызывается при старте игры, самой библиотекой Информа.
